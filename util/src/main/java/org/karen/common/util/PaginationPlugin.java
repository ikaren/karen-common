...
//				.getElements().get(element.getElements().size() - 1);
        ...
        XmlElement isNotNullPageSizeElement = new XmlElement("if"); //$NON-NLS-1$
        isNotNullPageSizeElement.addAttribute(new Attribute("test", " limitEnd != null and limitEnd > 0 ")); //$NON-NLS-1$ //$NON-NLS-2$
        isNotNullPageSizeElement.addElement(new TextElement("limit ${limitStart} , ${limitEnd}"));
